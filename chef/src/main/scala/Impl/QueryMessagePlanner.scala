...
case class QueryMessagePlanner(override val planContext: PlanContext) extends Planner[String] {
  override def plan(using PlanContext): IO[String] = {
    ...
    val sqlQuery = s"SELECT customer_name, dish_name, order_count, finish_state FROM customer.order_rec"
    ...
    val results: IO[List[(String, String, String, String)]] = jsonResultsIO.map { jsonResults =>
      jsonResults.map { jsonResult =>
        ...
        (customerName, dishName, orderCount, finishState)
      }
    }

    // Convert the list of tuples to a formatted string
    results.map { resultsList =>
      resultsList.map { case (customerName, dishName, orderCount, finishState) =>
