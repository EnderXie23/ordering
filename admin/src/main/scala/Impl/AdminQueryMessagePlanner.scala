...
case class AdminQueryMessagePlanner(override val planContext: PlanContext) extends Planner[String] {
  override def plan(using PlanContext): IO[String] = {
    ...
    val sqlQuery = s"SELECT chef_name, user_name, dish_name, quantity, price, state, orderID, orderPart FROM ${schemaName}.admin_log"
    val jsonResultsIO: IO[List[Json]] = readDBRows(sqlQuery, List.empty)

    // Convert the JSON results to a list of tuples
    val results: IO[List[(Option[String], String, String, String, String, String, String, String)]] = jsonResultsIO.map { jsonResults =>
      jsonResults.map { jsonResult =>
        ...
        (chefName, customerName, dishName, orderCount, price, finishState, orderID, orderPart)
      }
    }
    ...
    results.map { resultsList =>
      resultsList
        .filter { case (_, _, _, _, _, finishState,_,_) => finishState != "2" }
        .map { case (chefName, customerName, dishName, orderCount, price, finishState, orderID,orderPart) =>
          s"Chef:$chefName,Customer:$customerName,Dish:$dishName,Order Count:$orderCount,Price:$price,State:$finishState,OrderID:$orderID,OrderPart:$orderPart"
        }
        .mkString("\n")
    }
  }
