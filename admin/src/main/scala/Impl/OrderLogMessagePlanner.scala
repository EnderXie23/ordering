...
case class OrderLogMessagePlanner(log:String, override val planContext: PlanContext) extends Planner[String] {
  override def plan(using planContext: PlanContext): IO[String] = {
    ...
    def parseLog(log: String): Option[(String, String, String, String, String, String, String)] = {
      ...
      if (parts.length == 7) {
        ...
        val State = parts(6)
        Some((OrderID, OrderPart, ChefName, CustomerName, DishName, OrderCount, State))
      } else {
    ...
    parsedLog match {
      case Some((orderID, orderPart, chefName, customerName, dishName, orderCount, state)) =>
        // Start transaction
        startTransaction { // <-- Added transaction management
          for {
            // Validate inputs
            _ <- validateInputs(orderID, orderPart, chefName, customerName, dishName, orderCount, state) // <-- Ensure validation happens within transaction
            // Insert record into database
            _ <- {
              val query = s"INSERT INTO admin.admin_log (orderID, orderPart, user_name, chef_name, dish_name, quantity, state) VALUES (?, ?, ?, ?, ?, ?, ?)"
              val params = List(
  ...
  private def validateInputs(orderID:String, orderPart:String,chefName: String, customerName: String, dishName: String, orderCount: String, state: String): IO[Unit] = IO {
